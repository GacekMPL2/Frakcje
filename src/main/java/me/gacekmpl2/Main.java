...
import me.gacekmpl2.Commands.ReloadCommand;
...
import me.gacekmpl2.Utils.ConfigSave;
import me.gacekmpl2.Utils.Debug;
...
public final class Main extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        if (!getServer().getPluginManager().isPluginEnabled("PlaceholderAPI")) {
            Debug.sendError("&cNie wykryto pluginu PlaceholderAPI");
            Debug.sendError("&cPlugin nie bedzie poprawnie dzialal");
            return;
        }
        getLogger().info("Plugin sie wlaczyl");
        getCommand("easyhc").setExecutor((CommandExecutor) new ReloadCommand());
        getCommand("easyhc").setTabCompleter((TabCompleter) new TabCompliter());
